...
if (gend != "null" && coun != "null" && prof != "null") {
	crews = new ArrayList<Crew>();
	crews = cdao.getFilterCrews(prof, coun, gend);
}

if ( prof == "null" && coun == "null" && gend != "null") {
	crews = new ArrayList<Crew>();
	crews = cdao.getCrewsByGend(gend);
}

if (prof != "null" && coun == "null" && gend != "null") {
	crews = new ArrayList<Crew>();
	crews = cdao.getCrewsByProfGend(prof, gend);
}

if (prof == "null" && coun != "null" && gend != "null") {
	crews = new ArrayList<Crew>();
	crews = cdao.getCrewsByCounGend(coun, gend);
}
...
<html lang="en">
	...
	<%@ include file="navigationmenu_shipowner.jsp"%>
		...
		<div class="container">
			...
			<% if (crews.isEmpty()) { %>
				<div class="alert alert-warning text-center" role="alert">We couldn't find crews with the selected attributes. Please try with other options.</div>
			<% } %>
